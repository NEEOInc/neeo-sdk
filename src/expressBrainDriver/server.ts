...
import routeDatabase, {
  registerHandler as setDbHandler
} from './routes/database';
import routeDevice, {
  registerHandler as setDeviceHandler
} from './routes/deviceRoute';
...
const app = express()
  ...
  .use(<express.ErrorRequestHandler>((error, req, res, next) => {
    if (!next) {
