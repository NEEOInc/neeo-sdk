...
export function registerAdapterOnTheBrain(conf: {
  ...
  return axios.post(url, { name: adapterName, baseUrl }, REST_OPTIONS).then(({ data }) => data);
}

export function unregisterAdapterOnTheBrain(conf: { url: string; adapterName: string }) {
  ...
  return axios.post(url, { name: adapterName }, REST_OPTIONS).then(({ data }) => data);
}
