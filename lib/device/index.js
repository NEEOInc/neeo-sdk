...
module.exports.startServer = function(conf, _brainDriver) {
  ...
  const baseUrl = generateBaseUrl(conf);
...
function generateAdapterName(conf) {
  return 'src-' + uniqueName(conf.name);
}
...
function buildDevicesDatabase(conf, adapterName) {
  conf.devices.forEach((device) => {
    buildAndRegisterDevice(device, adapterName);
  });
...
function buildAndRegisterDevice(device, adapterName) {
  ...
  Database.register(deviceModel);
  brainDriver.registerDevice(deviceModel);
}
