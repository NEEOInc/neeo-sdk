...
module.exports.registerAdapterOnTheBrain = function(conf) {
  ...
  debug('registerAdapterOnTheBrain %o', { registerurl: url, name: conf.adapterName, baseUrl: conf.baseUrl });
...
module.exports.unregisterAdapterOnTheBrain = function(conf) {
  ...
  debug('unregisterAdapterOnTheBrain %s', { registerurl: url, name: conf.adapterName });
  return axios.post(url, { name: conf.adapterName })
    .then((response) => {
      return response.data;
    });
};
