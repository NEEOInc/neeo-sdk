...
const deviceBuilder = class DeviceBuilder {
   ...
   * @param {function} controller.deviceRemoved Callback function used when a device from this SDK is removed on the Brain. Can be used to stop listening to updates for that device.
   * The callback has one parameter:
   * **deviceId**: string identifying the device.
   ...
   * @param {String} configuration.name identifier of this element
   * @param {String} configuration.label optional, visible label in the mobile app or on the NEEO Remote
   * @param {String} configuration.type optional, can be range or binary, default range
   ...
   * @param {Object} controller An object which contains a getter callback function. This getter callback should return a list built by listBuilder so the App/TR2 can display the browse result as a list. If the getter callback encounters an error, you can build a list with a 'ListInfoItem' to inform the user about the error. The action callback will be called when an item is clicked.
   * @return {Object} DeviceBuilder
   * @example
   * .addDirectory({
