...
const inputchecker = require('./devicetype/inputchecker.js');
...
const deviceBuilder = class DeviceBuilder {
  constructor(name, uniqeString) {
    this.deviceidentifier = 'apt-' + validation.getUniqueName(name, uniqeString);
  ...
  /**
   * @function setType
   * @param {string} type supported device classes, either 'ACCESSOIRE', 'AVRECEIVER', 'DVB' (aka. satellite reciever), 'DVD' (aka. discplayer), 'GAMECONSOLE', 'LIGHT', 'MEDIAPLAYER', 'PROJECTOR', 'TV' or 'VOD' (aka. Video-On-Demand box like Apple TV, Fire TV...)
  ...
  build(adapterName) {
    ...
    if (this.timing && DeviceType.doesNotSupportTiming(this.type)) {
    ...
    if (DeviceType.needsInputCommand(this.type) && inputchecker.hasNoInputButtonsDefined(this.buttons)) {
   ...
   */
  supportsTiming() {
    return !DeviceType.doesNotSupportTiming(this.type);
  }
   ...
   * OptionalCallbacks: **powerOnNotificationFunction** - call this function with its deviceid when the device powers on.
   ...
   * deviceBuilder.registerSubscriptionFunction(function(updateCallback, optionalCallbackFunctions) {
