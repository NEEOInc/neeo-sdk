...
module.exports.buildDirectory = function(pathPrefix, param) {
  validateParameter(pathPrefix, param);
  const name = encodeURIComponent(param.name);
  const path = pathPrefix + name;
  const component = {
...
module.exports.buildRegister = function(pathPrefix) {
  if (!pathPrefix) {
    throw new Error('INVALID_PATHPREFIX');
  }
...
module.exports.buildDeviceSubscription = function(pathPrefix) {
  if (!pathPrefix) {
    throw new Error('INVALID_PATHPREFIX');
  }
