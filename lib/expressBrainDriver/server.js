...
const routeSecure = require('./routes/secure');
...
app.use(crypto.generateDecryptMiddleware(crypto.decrypt));
...
process.on('unhandledRejection', (error) => {
  debug('UNHANDLED_REJECTION', { msg: error.message, stack: error.stack });
});
...
function startExpress(conf, requestHandler) {
  if (expressServerRunning) {
    debug('IGNORE_REQUEST_EXPRESS_SERVER_ALREADY_RUNNING');
    return BluePromise.resolve();
  }

  return startServer(conf, requestHandler);
}

function startServer(conf, requestHandler) {
